...
class SACFluxAttnProcessor:
    ...
    def __init__(self, pipe, prompt, out_width, out_height, end_step, b_a_copyto_bp_ap_tau: float = 1.6, b_b_copyto_bp_b_tau: float = 1.0):
        super().__init__()
        self.pipe = pipe
        self.prompt = prompt
        self.block_idx = [0, 0]
    ...
    def __call__(
        ...
        if self.step_idx < inject_threshold and block_type == TransType.DOUBLE:
            import math
            ...
            # print(f"Max B->A: {max_b_to_a.mean().item():.4f}, Max native B'->A': {max_native_b_prime_to_a_prime.mean().item():.4f}")
            # 🌟 动态对齐 (Logit Alignment)
            aligned_b_prime_to_a_prime = clone_b_to_a + self.b_a_copyto_bp_ap_tau
            ...
            # 动作 3: B -> B 的自注意力也可以被克隆到 B' -> B‘，增强 B' 的自我结构感知（可选）
            # 1. 提取 B 关注 B 自身的注意力分数 (Query: B区域, Key: B区域)
            clone_b_to_b = img_sim[:, h_p:, :w_p, h_p:, :w_p].clone()
            # 2. 动态对齐/偏移 (利用你在 __init__ 中预先定义的 b_b_copyto_bp_b_tau)
            aligned_b_prime_to_b_prime = clone_b_to_b + self.b_b_copyto_bp_b_tau
            # 3. 将其注入到 B' 关注 B' 的位置 (Query: B'区域, Key: B'区域)
            img_sim[:, h_p:, w_p:, h_p:, w_p:] = aligned_b_prime_to_b_prime
